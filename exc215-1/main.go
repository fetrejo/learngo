...
Istanbul,500,10,5,1000000`
	...
	var (
		...
		avgs, avgbed, avgbath, avgp float64
	)
	...
	fmt.Printf("\n%s\n", strings.Repeat("=", 75))
	for i := range rows {
		avgs += float64(sizes[i])
		avgbed += float64(beds[i])
		avgbath += float64(baths[i])
		avgp += float64(prices[i])
	}
	fmt.Printf("%-15s", " ")
	fmt.Printf("%-15f", avgs/float64(len(rows)))
	fmt.Printf("%-15f", avgbed/float64(len(rows)))
	fmt.Printf("%-15f", avgbath/float64(len(rows)))
	fmt.Printf("%-15f", avgp/float64(len(rows)))
	fmt.Println()
}
